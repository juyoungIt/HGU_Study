...
Mat white_balancing(Mat input) {
    double ave_b, ave_g, ave_r;      // average of bgr channels
    int sum_b=0, sum_g=0, sum_r=0;   // sum of bgr channels
    int total_count = 0;             // total count of image
    Mat temp;
    Mat bgr_channels[3];
    split(input, bgr_channels);      // split b, g, r channel
    
    // calculate average value of b, g, r channel
    for(int i=0 ; i<temp.size().width ; i++) {
        for(int j=0 ; j<temp.size().height ; j++) {
            sum_b += bgr_channels[0].at<uchar>(j, i); // add current b channel pixel value
            sum_g += bgr_channels[1].at<uchar>(j, i); // add current g channel pixel value
            sum_r += bgr_channels[2].at<uchar>(j, i); // add current r channel pixel value
            total_count++; // count total pixels of image
        }
    }
    ave_b = (double)sum_b / total_count; // average value of b channel
    ave_g = (double)sum_g / total_count; // average value of g channel
    ave_r = (double)sum_r / total_count; // average value of r channel
    
    // execute white balancing
    for(int i=0 ; i<temp.size().width ; i++) {
        for(int j=0 ; j<temp.size().height ; j++) {
//            bgr_channels[0].at<uchar>(j, i) = (255 / bgr_channels[0].at<uchar>(j, i)) * ave_b; // processing - 1
//            bgr_channels[1].at<uchar>(j, i) = (255 / bgr_channels[1].at<uchar>(j, i)) * ave_g; // processing - 2
//            bgr_channels[2].at<uchar>(j, i) = (255 / bgr_channels[2].at<uchar>(j, i)) * ave_r; // processing - 3
