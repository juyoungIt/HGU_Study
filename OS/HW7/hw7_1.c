...
void Philosophers_PickUp(Philosophers *dp, int i)
{
	...
	pthread_mutex_lock(&dp->mutex); // lock the mutex
	dp->state[i] = HUNGRY;
	Philosophers_Test(dp, i);
	if(dp->state[i] != EATING)
		pthread_cond_wait(&dp->self[i], &dp->mutex);
	pthread_mutex_unlock(&dp->mutex); // unlock the mutex
}
...
void Philosophers_PutDown(Philosophers *dp, int i)
{
	...
	dp->state[i] = THINKING;
	// test left and right
	Philosophers_Test(dp, (i+dp->no_phil-1)%dp->no_phil); // left
	Philosophers_Test(dp, (i+1)%dp->no_phil); // right
	pthread_mutex_unlock(&dp->mutex); // unlock the mutex
}
